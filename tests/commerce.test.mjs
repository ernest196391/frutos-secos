...
test("pickup message excludes delivery address",()=>{const t=orderMessage({...order,address:"NO MOSTRAR",municipality:"NO MOSTRAR"},{orderTitle:"FRUTOS SECOS COLO",pickupPoint:"Frutos Secos Colo"});assert.ok(t.includes("*Producto* ×2"));assert.ok(!t.includes("NO MOSTRAR"))});
test("voucher surfaces reference, total and confirmation steps",()=>{const t=orderMessage(order,{orderTitle:"FRUTOS SECOS COLO",pickupPoint:"Frutos Secos Colo"});assert.ok(t.includes("Pedido #COLO-ABCD"));assert.ok(t.includes("*TOTAL · 1,600 CUP*"));assert.ok(t.includes("PENDIENTE DE CONFIRMACIÓN"));assert.ok(t.includes("1. Disponibilidad de los productos"));assert.ok(t.includes("3. Horario estimado de recogida"))});
