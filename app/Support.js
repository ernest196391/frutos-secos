...
export default function Support({onAdd,onSet,onClear,cart={}}){
  ...
  useEffect(()=>{let active=true;fetch("/api/commerce/config",{cache:"no-store"}).then(async r=>{const data=await r.json();if(!r.ok)throw Error(data?.error||"CONFIG_ERROR");if(active)setConfig(data)}).catch(()=>{if(active){setConfig({assistantReady:false});setError("No pudimos verificar la conexión del asistente. Puedes reintentar enviando tu mensaje.")}});return()=>{active=false}},[]);
  ...
  return <div className="assistantV2"><div className="supportDock">{nudge&&!open&&<button className="assistantNudge" onClick={openPanel}><span className="assistantNudgeText">{AI.nudge}</span><span className="assistantNudgeWave" aria-hidden="true">👋</span></button>}<button ref={launcher} className="assistantLaunch" aria-label={"Abrir "+AI.name} onClick={openPanel}>{avatarNode()}</button>{config?.whatsapp&&<a className="waLaunch" href={config.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"><Glyph name="whatsapp"/></a>}</div>
  {open&&<div className="assistantBackdrop" onClick={close}><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={AI.name} className="assistantPanel" onClick={e=>e.stopPropagation()} onKeyDown={keys}>
    <div className="assistantBrandBar"><div><strong>{tenant.name}</strong><span> · {tenant.tagline}</span></div><button className="assistantClose" onClick={close}>×</button></div>
    <div className="assistantIdentity"><div className="assistantAvatar">{avatarNode()}</div><div><b>{AI.name}</b><small className={config?.assistantReady?"assistantOnline":"assistantConnecting"}>{config===null?"Conectando…":config?.assistantReady?"● En línea":"Conexión por validar"}</small><small>{AI.role}</small></div></div>
    <div className="assistantMessages" aria-live="polite"><p><b>{AI.greeting}</b><br/>{AI.greetingSub}</p><p>{AI.intro}</p>
      {cartCount>0&&<div className="assistantCartStrip"><span>🛒 {cartCount} {cartCount===1?"producto":"productos"} en tu carrito</span>{confirmClear?<span className="assistantClearAsk">¿Vaciar todo?<button className="cancelOrder" onClick={()=>{onClear?.();setConfirmClear(false)}}>Sí, vaciar</button><button onClick={()=>setConfirmClear(false)}>No</button></span>:<><a href="/checkout">Revisar pedido →</a><button className="assistantClearBtn" onClick={()=>setConfirmClear(true)}>Vaciar</button></>}</div>}
      {messages.map((m,i)=><article key={i} className={m.role}><p>{m.text}</p>{m.attachments?.some(a=>a.url)&&<div className="assistantMessageMedia">{m.attachments.filter(a=>a.url).map((a,n)=><img key={n} src={a.url} alt="Foto enviada"/>)}</div>}{m.products?.length>0&&<><div className="assistantProductGrid">{m.products.map(p=>{const inCart=cart[p.id]||0;return <div className="assistantProduct" key={p.id}><div className="assistantProductImage"><img src={p.img} alt={p.n}/></div><div className="assistantProductInfo"><b>{p.n}</b><span>{p.d}</span><strong>{money(p.p)}</strong>{inCart?<div className="assistantQty"><button aria-label={"Quitar una unidad de "+p.n} onClick={()=>onSet?.(p.id,inCart-1)}>−</button><span>{inCart} en carrito</span><button aria-label={"Añadir otra unidad de "+p.n} onClick={()=>onAdd(p.id)}>＋</button></div>:<button onClick={()=>onAdd(p.id)}>Añadir al carrito</button>}</div></div>})}</div>{(()=>{const faltan=m.products.filter(p=>!cart[p.id]);return m.products.length>1&&faltan.length>1&&<button className="assistantMultiAdd" onClick={()=>faltan.forEach(p=>onAdd(p.id))}>＋ Añadir {faltan.length===m.products.length?`los ${faltan.length} productos`:`los ${faltan.length} que faltan`}</button>})()}</>}{m.checkout?.url&&<a className="assistantCheckoutBtn" href={m.checkout.url}>Ir al checkout <span aria-hidden="true">→</span></a>}{m.contact?.url&&<a className="assistantContactBtn" href={m.contact.url} target="_blank" rel="noopener noreferrer"><Glyph name="whatsapp"/> Escribir por WhatsApp</a>}</article>)}
      {busy&&<p className="assistantTyping" role="status">{AI.name} está buscando…</p>}<div ref={end}/></div>
    {messages.length===0&&<div className="assistantQuick">{quick.map(([icon,title,sub,prompt])=><button key={title} type="button" onClick={()=>ask(prompt,[])}><Glyph name={icon}/><span>{title}<small>{sub}</small></span></button>)}{config?.whatsapp&&<a className="whatsappAction" href={config.whatsapp} target="_blank" rel="noopener noreferrer"><Glyph name="whatsapp"/> Confirmar por WhatsApp<small>Continúa tu compra con una persona</small></a>}</div>}
    {error&&<p className="assistantError" role="alert">{error}</p>}
    ...
    <form className="assistantComposer" onSubmit={send}><label htmlFor="assistant-q">Tu consulta</label><textarea id="assistant-q" maxLength={1000} value={q} onChange={e=>setQ(e.target.value)} placeholder={recording?"Escuchando…":"Escribe tu mensaje..."}/>{files.length>0&&<div className="assistantFiles">{files.map((f,i)=><div className="assistantFilePreview" key={i}>{previews[i]?<img src={previews[i]} alt="Vista previa"/>:<span>{f.name}</span>}<button type="button" onClick={()=>setFiles(v=>v.filter((_,n)=>n!==i))}>×</button></div>)}</div>}<div className="assistantTools"><label className="fileControl"><Glyph name="attach"/><input type="file" multiple accept=".jpg,.jpeg,.png,.webp,.pdf,.txt" onChange={e=>{addFiles(e.target.files);e.target.value=""}}/></label><button className="cameraControl" type="button" onClick={openCamera}><Glyph name="camera"/></button><button className={recording?"micControl recording":"micControl"} type="button" onClick={dictate} aria-pressed={recording} aria-label={recording?"Detener dictado":"Dictar por voz"}><Glyph name="mic"/>{recording&&<span className="micWave" aria-hidden="true"><i/><i/><i/></span>}</button><button type="submit" disabled={busy||(!recording&&!q.trim()&&!files.length)} aria-label={recording?"Finalizar transcripción y enviar":"Enviar consulta"}><Glyph name="send"/></button></div></form>
    <div className="assistantTagline">{AI.closing}</div>
    {cameraOpen&&<div className="assistantCamera"><video ref={videoRef} autoPlay playsInline muted/><div><button type="button" onClick={stopCamera}>Cancelar</button><button type="button" className="cameraShot" onClick={takePhoto}>Tomar foto</button></div></div>}
  </section></div>}</div>
}